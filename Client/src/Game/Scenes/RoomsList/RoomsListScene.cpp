    ...
    namespace Game
    {
        namespace Scenes
        {
            ...
            std::string RoomsList::handleEvent(gd::Window &window, gd::Event &event)
            {
                ...
                if (event.keyBoard.getKeyState(gd::KeyBoard::Key::Escape) == gd::KeyBoard::State::Pressed) return "menu";
                ...
                if (event.joyStick.isConnected()) {
                    if (event.joyStick.getYAxisPosition(false) < -50) (_selectColumn == 0) ? _moveSelectIndexLink(-1) : _moveSelectIndexRoomGameSlot(window, -1);
                    if (event.joyStick.getYAxisPosition(true) > 50) (_selectColumn == 0) ? _moveSelectIndexLink(1) : _moveSelectIndexRoomGameSlot(window, 1);
                    if (event.joyStick.getXAxisPosition(false) < -50) _moveSelectedColumn(window, -1);
                    if (event.joyStick.getXAxisPosition(true) > 50) _moveSelectedColumn(window, 1);
